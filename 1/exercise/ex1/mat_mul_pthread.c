...
int main(int argc, const char *argv[]){
    if(argc >= 3){
        ...
        param_t *work;
        work = (param_t *) malloc(num_threads * sizeof(param_t));
        int count = 0;
        //
        for(i = 0; i< num_threads;i++)
        {
            work[i].id = count;
            work[i].rows = &num_rows_per_thread[i];
            work[i].m_a = matrix_a;
            work[i].m_b = matrix_b;
            work[i].m_r = result;
            work[i].dim = dim;
            count = count + num_rows_per_thread[i];
        }

        for (i = 0; i< num_threads; i++)
        {
            pthread_create(&thread[i], NULL, worker, (void *) &work[i]);
        }
        //////////////////////////////////////////////////////////
